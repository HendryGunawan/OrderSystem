...
class GoodReceiptRollingDoor extends Model
{
    /**
     ...
     * @var string
     */
    protected $table = 'good_receipt_rolling_doors';
    ...
    public static function getData($id){
        $query = self::where('good_receipt_rolling_doors.id', $id)
                     ->join('items', 'good_receipt_rolling_doors.item_id', '=', 'items.id')
                     ->select('good_receipt_rolling_doors.*', 'items.name')
                     ->first();
        if(!empty($query)){
            return $query;
        }else{
    ...
    public static function getDataAll(){
        $query = self::where('good_receipt_rolling_doors.delete_flag', 0)
                     ->select('good_receipt_rolling_doors.id', 'good_receipt_rolling_doors.name')
                     ->get();
        if(!empty($query)){
            return $query;
        }else{
    ...
    public static function getDataTable(){
        $query =  self::select('good_receipt_rolling_doors.*', 'items.name')
                      ->where('good_receipt_rolling_doors.delete_flag', 0)
                      ->join('items', 'good_receipt_rolling_doors.item_id', '=', 'items.id')
                      ->orderBy('id', 'asc')
                      ->get();
        if(!empty($query))
    ...
    public static function getQuota($item_code)
    {
        ...
        $usage = GoodUsageRollingDoorDetail::getDataUsage($item_code);

        if(!empty($usage))
        {
            $usage_quota = $usage['SUM(good_usage_rolling_door_details.length)'];
        }      
