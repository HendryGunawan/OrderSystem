...
@section('content')
<a href="{{ route('rolling_door_sparepart_order') }}">
              ...
              <tbody>
                  ...
                  for($i=1;$i<=12;$i++)
                  {
                    ...
                    <tr data-id="<?php echo $i ?>">
                      ...
                      <td>
                          <select name="order[<?php echo $i ?>][rolling_door_sparepart_id]" class="form-control ItemName" onclick="checkprice(this)" <?php echo $i==1? 'required':''?>>
...
function checkprice(row)
{
    ...
    $.ajax({
        url: '{{ route("rolling_door_sparepart_price") }}?id='+row.value,
        method: 'GET',
        dataType: 'JSON',
