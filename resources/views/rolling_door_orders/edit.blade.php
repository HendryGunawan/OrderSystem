...
<div class="container">
  <div id="Checkout" class="inline">
      <h1>Edit Rolling Door Order</h1>
      <form id="folding-gate-sparepart-order-edit" method="POST" action="{{ route('rolling_door_order_edit_post') }}" role="form">
      {{ csrf_field() }}
              ...
              <tbody>
                  ...
                  foreach($child as $key => $child_value)
                  {
                    ...
                    <tr data-id="<?php echo $key ?>">
                      <td>
                          <select name="order[<?php echo $key ?>][rolling_door_id]" class="form-control" onclick="checkprice(this)">
                              <option value=""></option>
                              <?php
