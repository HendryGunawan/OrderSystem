...
<div class="container">
  <div id="Checkout" class="inline">
      <h1>Hapus Pesanan Folding Gate</h1>
      <form id="folding-gate-order-add" method="POST" action="{{ route('folding_gate_order_delete_post') }}" role="form">
      {{ csrf_field() }}
              ...
              <tbody>
                  ...
                  foreach($child as $key => $child_value)
                  {
                    ...
                    <tr data-id="<?php echo $key ?>">
                      ...
                      <td>
                          <input type="text" class="form-control" name="order[<?php echo $key ?>][ItemName]" min="0" value="<?php echo $child_value['ItemName'] ?>" readonly>
                      </td>
                      <td>
